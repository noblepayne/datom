...
(defn -main [& args]
  (let [sys (init-system!)
    ...
    (when-let [api-port (config/parse-int (config/get-env "DATOM_API_PORT" ""))]
      (println "Starting datom-api on" (str host ":" api-port))
      (api/start-server sys {:port api-port :host host}))
