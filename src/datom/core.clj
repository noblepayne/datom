...
(defn search
  ...
  [sys query & {:keys [top author expand raw] :as opts}]
  (apply query/search sys query (mapcat identity opts)))

(defn answer
  "Search and return a human-readable string."
  [sys query & {:as opts}]
  (apply query/answer sys query (mapcat identity opts)))
...
(defn -main [& args]
  (if (empty? args)
    (do (println "Usage: datom <search query> [--author name] [--top n] [--expand n] [--source type]"))
